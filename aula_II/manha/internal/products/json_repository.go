...
func NewJsonRepository(db store.Store) Repository {
...
func (r *jsonRepository) Store(p Product) (Product, error) {
	lastID, err := r.LastID()
	if err != nil {
		return Product{}, err
	}
...
*/
func (r *jsonRepository) Update(p Product) (Product, error) {
	...
	for i := range ps { // Este For percorrerá a lista dos elementos criados no array para buscar o elemento com o Id que já existe
		if ps[i].ID == p.ID { // Caso encontre esse Id ...
			p.ID = p.ID    // ... o Id do novo produto será o mesmo do já existente (basicamente, o Id que passamos substituirá o já existente, só que são iguais)...
